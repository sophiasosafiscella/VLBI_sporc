...
#    eq_timing_model = ec_timing_model.as_ICRS()  # Equatorial coordinates
    ...
    # ------------------------------RAJ------------------------------
    VLBI_RAJ = Angle(VLBI_data.loc[PSR, "VLBI_RAJ"])
    ref_RAJ = Angle(f"{int(VLBI_RAJ.hms[0])}h{int(VLBI_RAJ.hms[1])}m{int(VLBI_RAJ.hms[2])}s")

    # VLBI
    VLBI_deltaRAJ_ms = (Angle(VLBI_data.loc[PSR, "VLBI_RAJ"]) - ref_RAJ).hms[2] * 1000.0
    VLBI_RAJ_err_ms = Angle(VLBI_data.loc[PSR, "VLBI_RAJ_err"]).hms[2] * 1000.0
    ...
    # ------------------------------DECJ------------------------------
    VLBI_DECJ = Angle(VLBI_data.loc[PSR, "VLBI_DECJ"])
    ref_DECJ = Angle(f"{int(VLBI_DECJ.dms[0])}d{int(abs(VLBI_DECJ.dms[1]))}m{int(abs(VLBI_DECJ.dms[2]))}s")

    # VLBI
    VLBI_deltaDECJ_ms = (Angle(VLBI_data.loc[PSR, "VLBI_DECJ"]) - ref_DECJ).dms[2] * 1000.0
    VLBI_DECJ_err_ms = Angle(VLBI_data.loc[PSR, "VLBI_DECJ_err"]).dms[2] * 1000.0
    ...
    #------------------------------Proper Motion------------------------------
    VLBI_DECJ = ufloat(Angle(VLBI_data.loc[PSR, "VLBI_DECJ"]).rad, Angle(VLBI_data.loc[PSR, "VLBI_DECJ_err"]).rad)
