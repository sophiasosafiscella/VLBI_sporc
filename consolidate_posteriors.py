...
posteriors_dir: str = f"./results/timing_posteriors_frame_tieframe_tie/{PSR_name}"
posterior_files = glob(f"{posteriors_dir}/*posterior.npy")

# Array set to zero but to be replaced with the posteriors
res_np = np.zeros((len(posterior_files), 6), dtype=float)
...
result_df = pd.DataFrame(data=res_np, index=idx_np, columns=["RAJ", "DECJ", "PMRA", "PMDEC", "PX", "posterior"]).sort_index()
